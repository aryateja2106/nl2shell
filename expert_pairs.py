...
Total: 1009 expert-quality pairs
"""


EXPERT_PAIRS: list[tuple[str, str]] = [
    ("redirect only the stderr of a script through a grep filter while keeping stdout intact", "./build.sh 3>&1 1>&2 2>&3 | grep -i fail"),
    ("swap stderr and stdout streams using a temporary file descriptor 3", "exec 3>&1; ./script.sh 2>&1 1>&3 3>&- | sed 's/^/ERR: /' >&2; exec 3>&-"),
    ("check exit code of first command in a pipe", "gzip < big.log | wc -c; echo \"gzip exit: ${PIPESTATUS[0]}\""),
    ("fail if any command in a pipeline fails", "set -o pipefail && make build | tee build.log"),
    ("capture all pipeline exit codes after running", "cmd1 | cmd2 | cmd3; echo \"exits: ${PIPESTATUS[*]}\""),
    ("assert pipeline succeeded and print status array", "pg_dump mydb | gzip > mydb.sql.gz; [[ ${PIPESTATUS[0]} -eq 0 ]] || echo 'pg_dump failed'"),
    ("tee command output to two different files at once", "./deploy.sh | tee deploy.log >(grep ERROR > errors.log)"),
    ("tee stdout to file while also sending to a log aggregator", "./app.sh 2>&1 | tee -a app.log | logger -t myapp"),
    ("fan output to three consumers via process substitution", "cat access.log | tee >(grep 200 | wc -l > ok.txt) >(grep 500 | wc -l > err.txt) >/dev/null"),
    ("tee stderr to a file while stdout continues to terminal", "{ ./run.sh 2>&1 1>&3 | tee errors.log 1>&2; } 3>&1"),
    ("diff two command outputs and fail if they differ", "diff <(./expected.sh 2>/dev/null) <(./actual.sh 2>/dev/null) || exit 1"),
    ("feed process substitution output into while loop", "while IFS= read -r line; do echo \"got: $line\"; done < <(kubectl get pods 2>/dev/null)"),
    ("capture stderr of process substitution separately", "diff <(cmd1 2>cmd1_err.log) <(cmd2 2>cmd2_err.log)"),
    ("write stdout to one file and stderr to another without shell builtins", "./app 1>out.log 2>err.log"),
    ("redirect only fd 3 to a file for custom logging", "exec 3>debug.log; echo 'debug info' >&3; exec 3>&-"),
    ("duplicate stdout to a log file using exec redirect", "exec > >(tee -a session.log) 2>&1"),
    ("send stdout to file and stderr to a separate command", "./run.sh 2> >(grep -v DeprecationWarning >&2)"),
    ("capture command output to variable while still printing it", "output=$(./check.sh | tee /dev/stderr)"),
    ("print progress to stderr and final result to stdout", "{ echo 'working...' >&2; sleep 1; echo 'done'; }"),
    ("redirect stdout to stderr for a single echo", "echo 'this goes to stderr' >&2"),
    ("create a worktree for a hotfix branch without switching", "git worktree add ../hotfix-tree hotfix/critical-bug"),
    ("list all active worktrees with their paths", "git worktree list --porcelain"),
    ("remove a worktree and prune stale references", "git worktree remove ../hotfix-tree && git worktree prune"),
    ("find the commit hash before an accidental reset", "git reflog | grep 'reset\\|checkout' | head -20"),
    ...
    ("search commit messages for a pattern across all branches", "git log --all --oneline --grep='hotfix' --regexp-ignore-case"),
    ("find all commits that added or removed a function name", "git log -S 'authenticate_user' --all --oneline --source"),
    ...
    ("list all files that were changed in a specific commit", "git diff-tree --no-commit-id -r --name-only abc1234"),
    ("clone a repo and initialize all submodules recursively", "git clone --recurse-submodules https://github.com/org/repo.git"),
    ("update all submodules to their latest tracked commit", "git submodule update --init --recursive --remote"),
    ("show which submodule commit is checked out", "git submodule status --recursive"),
    ("enable sparse checkout and fetch only a specific directory", "git sparse-checkout init --cone && git sparse-checkout set src/api"),
    ("add another directory to an existing sparse checkout", "git sparse-checkout add docs/architecture"),
    ("get all pods across all namespaces with node assignment", "kubectl get pods -A -o wide"),
    ("follow logs of a crashing pod and restart on failure", "kubectl logs -f --previous deployment/api-server -n production"),
    ("describe a failing pod to see events and status", "kubectl describe pod api-server-6d8f7b9-xkpqz -n production"),
    ("exec into a running pod container", "kubectl exec -it deployment/api-server -n production -- /bin/sh"),
    ("get all pods in CrashLoopBackOff state", "kubectl get pods -A --field-selector=status.phase=Running | grep CrashLoop"),
    ("watch pod rollout status in real time", "kubectl rollout status deployment/api-server -n production --timeout=5m"),
    ("port-forward a service to localhost for debugging", "kubectl port-forward svc/postgres 5432:5432 -n staging"),
    ("get resource limits and requests for all pods in a namespace", "kubectl get pods -n production -o json | jq '.items[].spec.containers[].resources'"),
    ("force delete a stuck terminating pod", "kubectl delete pod stuck-pod-xyz -n production --grace-period=0 --force"),
    ("apply a manifest and wait for rollout to complete", "kubectl apply -f k8s/deployment.yaml && kubectl rollout status deployment/api-server -n production"),
    ("list all running EC2 instances with their IPs", "aws ec2 describe-instances --filters 'Name=instance-state-name,Values=running' --query 'Reservations[*].Instances[*].[InstanceId,PublicIpAddress,Tags[?Key==`Name`].Value|[0]]' --output table"),
    ("tail CloudWatch logs for a Lambda function", "aws logs tail /aws/lambda/my-function --follow --format short"),
    ("copy a file to S3 with server-side encryption", "aws s3 cp secrets.json s3://my-bucket/config/secrets.json --sse aws:kms"),
    ("invoke a Lambda function and show the response", "aws lambda invoke --function-name my-function --payload '{\"key\":\"value\"}' /tmp/out.json && cat /tmp/out.json"),
    ("get a secret value from Secrets Manager", "aws secretsmanager get-secret-value --secret-id prod/db/password --query SecretString --output text"),
    ("list all Cloud Run services across all regions", "gcloud run services list --platform managed --format='table(name,region,status.url)'"),
    ("stream Cloud Run logs in real time", "gcloud beta run services logs tail my-service --region us-central1"),
    ("deploy a container image to Cloud Run", "gcloud run deploy my-service --image gcr.io/project/image:latest --region us-central1 --allow-unauthenticated"),
    ("get the external IP of a GKE load balancer service", "gcloud compute forwarding-rules list --filter='name~my-service' --format='value(IPAddress)'"),
    ("set a Cloud Run environment variable without redeploying image", "gcloud run services update my-service --set-env-vars DB_URL=postgres://host/db --region us-central1"),
    ("run a SQL query from the command line without interactive prompt", "psql -U myuser -d mydb -c 'SELECT count(*) FROM users WHERE active = true;'"),
    ("export a table to CSV from psql", "psql -U myuser -d mydb -c '\\COPY users TO STDOUT WITH CSV HEADER' > users.csv"),
    ("run a SQL file against a database non-interactively", "psql -U myuser -d mydb -f migrations/001_add_index.sql"),
    ("show all table sizes in a PostgreSQL database", "psql -U myuser -d mydb -c 'SELECT relname, pg_size_pretty(pg_total_relation_size(oid)) FROM pg_class WHERE relkind=\\'r\\' ORDER BY pg_total_relation_size(oid) DESC LIMIT 20;'"),
    ("list all running queries in PostgreSQL", "psql -U myuser -d mydb -c 'SELECT pid, now()-query_start AS duration, state, query FROM pg_stat_activity WHERE state != \\'idle\\' ORDER BY duration DESC;'"),
    ("kill a long-running PostgreSQL query by pid", "psql -U myuser -d mydb -c 'SELECT pg_terminate_backend(12345);'"),
    ("query a SQLite database file from the command line", "sqlite3 app.db 'SELECT * FROM sessions WHERE expires_at < datetime(\"now\");'"),
    ("export SQLite table to CSV", "sqlite3 -header -csv app.db 'SELECT * FROM events ORDER BY created_at DESC LIMIT 1000;' > events.csv"),
    ...
    ("run a MySQL query from the command line", "mysql -u root -p'$DB_PASS' mydb -e 'SELECT table_name, table_rows FROM information_schema.tables WHERE table_schema = database();'"),
    ("dump a single MySQL table to SQL file", "mysqldump -u root -p'$DB_PASS' mydb users > users_backup.sql"),
    ("import a SQL dump into MySQL", "mysql -u root -p'$DB_PASS' mydb < backup.sql"),
    ("get all keys matching a pattern in Redis", "redis-cli --scan --pattern 'session:*' | head -20"),
    ("flush all keys in a specific Redis database", "redis-cli -n 2 FLUSHDB"),
    ("monitor live Redis commands in real time", "redis-cli MONITOR | grep -i 'set\\|get' | head -50"),
    ("check Redis memory usage and eviction stats", "redis-cli INFO memory | grep -E 'used_memory_human|maxmemory_human|evicted_keys'"),
    ("count documents in a MongoDB collection from the shell", "mongosh mydb --eval 'db.users.countDocuments({active: true})'"),
    ("export a MongoDB collection to JSON via mongosh", "mongosh mydb --eval 'JSON.stringify(db.events.find({},{_id:0}).toArray())' --quiet > events.json"),
    ("generate a 4096-bit RSA private key", "openssl genrsa -out private.key 4096"),
    ("create a certificate signing request from an existing key", "openssl req -new -key private.key -out request.csr -subj '/CN=example.com/O=MyOrg/C=US'"),
    ("view the details of an x509 certificate", "openssl x509 -in cert.pem -noout -text"),
    ("verify a certificate matches its private key", "diff <(openssl x509 -in cert.pem -pubkey -noout) <(openssl rsa -in private.key -pubout 2>/dev/null)"),
    ("convert a PEM certificate to DER format", "openssl x509 -in cert.pem -outform DER -out cert.der"),
    ("encrypt a file for a specific recipient with GPG", "gpg --encrypt --recipient user@example.com --output secret.gpg plaintext.txt"),
    ...
    ("generate an ed25519 SSH key with a comment", "ssh-keygen -t ed25519 -C 'user@host-$(date +%Y%m%d)' -f ~/.ssh/id_ed25519_new"),
    ...
    ("copy your public key to a remote server", "ssh-copy-id -i ~/.ssh/id_ed25519.pub user@remote.host"),
    ...
    ("verify a file against a published SHA-256 checksum", "echo 'abc123...  installer.pkg' | shasum -a 256 -c -"),
    ("generate MD5 checksums for all files in a directory", "find . -type f -print0 | xargs -0 md5 -r 2>/dev/null | sort > checksums.md5"),
    ("lock down SSH private key permissions", "chmod 600 ~/.ssh/id_ed25519 && chmod 700 ~/.ssh"),
    ("recursively set correct web-root ownership and permissions", "find /var/www/html -type d -exec chmod 755 {} + && find /var/www/html -type f -exec chmod 644 {} +"),
    ("trace all file-open syscalls made by a running process", "strace -p $(pgrep -n nginx) -e trace=openat,open 2>&1 | head -50"),
    ("trace network syscalls of a new process invocation", "strace -e trace=network,socket -f ./server 2>&1 | head -100"),
    ("summarise syscall counts and time for a command", "strace -c -f ./app 2>&1 | tail -20"),
    ("trace file opens on macOS using dtruss", "sudo dtruss -f -t open_nocancel -p $(pgrep -n node) 2>&1 | head -50"),
    ("record a CPU perf profile for 10 seconds", "sudo perf record -F 99 -g -p $(pgrep app) -- sleep 10"),
    ("generate a flamegraph from a perf.data recording", "sudo perf script | stackcollapse-perf.pl | flamegraph.pl > flamegraph.svg"),
    ("show top CPU-consuming functions in perf report", "sudo perf report --stdio --sort=dso,symbol | head -40"),
    ("run a command three times and show real elapsed time each run", "for i in 1 2 3; do { time ./benchmark; } 2>&1 | grep real; done"),
    ("measure wall clock and CPU time of a pipeline", "{ time (gzip -c largefile | wc -c); } 2>&1"),
    ("benchmark two implementations and compare", "hyperfine --warmup 5 './impl_a input.dat' './impl_b input.dat' --export-markdown results.md"),
    ("run a program under valgrind memcheck and show leak summary", "valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes ./app 2>&1 | tail -30"),
    ("profile heap allocations with valgrind massif", "valgrind --tool=massif --pages-as-heap=yes ./app && ms_print massif.out.* | head -60"),
    ("check for memory leaks on macOS with leaks tool", "leaks --atExit -- ./app 2>&1 | grep 'leaks for'"),
    ("run heap profiling with gperftools on Linux", "HEAPPROFILE=/tmp/heap.prof LD_PRELOAD=/usr/lib/libprofiler.so ./app"),
    ...
    ("find which process is hammering disk I/O right now", "sudo iotop -o -P -b -n 1 | sort -k10 -rn | head -10"),
    ("measure latency of disk operations with fio quick test", "fio --name=randread --ioengine=libaio --iodepth=16 --rw=randread --bs=4k --size=256M --runtime=10 --filename=/tmp/fio_test"),
    ("open crontab for editing", "EDITOR=nano crontab -e"),
    ("list all cron jobs for the current user", "crontab -l 2>/dev/null || echo 'no crontab for $USER'"),
    ("remove all cron jobs for the current user", "crontab -r"),
    ("back up current crontab before modifying it", "crontab -l > ~/crontab.bak.$(date +%Y%m%d) 2>/dev/null"),
    ("schedule a script to run every day at 2:30 AM", "(crontab -l 2>/dev/null; echo '30 2 * * * /opt/scripts/daily.sh >> /var/log/daily.log 2>&1') | crontab -"),
    ("run a job every 15 minutes and log output", "(crontab -l 2>/dev/null; echo '*/15 * * * * /opt/scripts/check.sh >> /var/log/check.log 2>&1') | crontab -"),
    ("enable a systemd service to start on boot", "sudo systemctl enable --now myapp.service"),
    ("disable a systemd service and stop it immediately", "sudo systemctl disable --now myapp.service"),
    ("view the last 100 lines of a service journal", "journalctl -u myapp.service -n 100 --no-pager"),
    ("create a minimal systemd unit file for a web service", "printf '[Unit]\\nDescription=My Application\\nAfter=network.target\\n\\n[Service]\\nUser=appuser\\nWorkingDirectory=/opt/myapp\\nExecStart=/opt/myapp/bin/server\\nRestart=always\\nRestartSec=5\\n\\n[Install]\\nWantedBy=multi-user.target\\n' | sudo tee /etc/systemd/system/myapp.service > /dev/null"),
    ("load and start a launchd plist agent immediately", "launchctl load -w ~/Library/LaunchAgents/com.example.myapp.plist"),
    ("unload and stop a launchd agent", "launchctl unload -w ~/Library/LaunchAgents/com.example.myapp.plist"),
    ("check if a launchd agent is running", "launchctl list | grep com.example.myapp"),
    ("run a command every 5 minutes via a launchd plist", "printf '<?xml version=\"1.0\" encoding=\"UTF-8\"?>\\n<plist version=\"1.0\"><dict>\\n<key>Label</key><string>com.example.heartbeat</string>\\n<key>ProgramArguments</key><array><string>/opt/scripts/heartbeat.sh</string></array>\\n<key>StartInterval</key><integer>300</integer>\\n<key>RunAtLoad</key><true/>\\n</dict></plist>' > ~/Library/LaunchAgents/com.example.heartbeat.plist && launchctl load -w ~/Library/LaunchAgents/com.example.heartbeat.plist"),
    ("schedule a one-time job to run in 10 minutes", "echo '/opt/scripts/reindex.sh >> /tmp/reindex.log 2>&1' | at now + 10 minutes"),
    ("list all pending at jobs", "atq"),
    ("remove a pending at job by job number", "atrm 3"),
    ("start a supervisor-managed process and check its status", "supervisorctl start myapp && supervisorctl status myapp"),
    ("tail supervisor log for a specific program", "supervisorctl tail -f myapp stderr"),
    ("reload supervisor config and restart all programs", "supervisorctl reread && supervisorctl update && supervisorctl restart all"),
    ...
    ("stop and remove a Homebrew service from auto-start", "brew services stop redis && brew services cleanup"),
    ...
    ("create an isolated virtual environment with uv and install deps", "uv venv .venv && uv pip install -r requirements.txt"),
    ("install a Python CLI tool globally with pipx", "pipx install httpie"),
    ("upgrade all pipx-installed tools", "pipx upgrade-all"),
    ("sync project dependencies precisely from pyproject.toml with uv", "uv pip sync requirements/prod.txt"),
    ("install a package and its recommended dependencies on Debian", "sudo apt-get install -y --install-recommends git-lfs"),
    ...
    ("install a specific Node.js version with nvm and use it", "nvm install 22 && nvm use 22 && nvm alias default 22"),
    ("set a global Python version with pyenv", "pyenv install 3.13.0 && pyenv global 3.13.0"),
    ("update the stable Rust toolchain and all components", "rustup update stable && rustup component add clippy rustfmt"),
    ...
    ("check what woke the Mac from sleep last time", "pmset -g log | grep -E 'Wake|Sleep' | tail -20"),
    ("check if a binary is allowed by Gatekeeper", "spctl --assess --verbose /Applications/SomeApp.app"),
    ("show code signing details of an application", "codesign -dvv /Applications/SomeApp.app 2>&1"),
    ("verify the code signature of a binary is intact", "codesign --verify --strict --verbose=2 /usr/local/bin/mytool"),
    ("sign a binary with your developer identity", "codesign --force --sign 'Developer ID Application: Acme Corp' /path/to/binary"),
    ("show all Gatekeeper rules currently in effect", "spctl --list"),
    ("show kernel logs from the last 5 minutes", "log show --predicate 'process == \"kernel\"' --last 5m --style compact"),
    ("stream live system log filtered to a subsystem", "log stream --predicate 'subsystem == \"com.apple.network\"' --level debug"),
    ("find recent crashes for a specific process", "log show --predicate 'process == \"Safari\" AND messageType == fault' --last 1h"),
    ("list all extended attributes on a file", "xattr -l ~/Downloads/SomeApp.dmg"),
    ("remove quarantine flag from a downloaded file", "xattr -d com.apple.quarantine ~/Downloads/SomeApp.dmg"),
    ("strip all extended attributes from a directory recursively", "xattr -cr ~/Downloads/untrusted_folder"),
    ("resize an image to 800px wide preserving aspect ratio", "sips --resampleWidth 800 photo.jpg"),
    ("batch convert all PNGs to JPEG in current directory", "sips -s format jpeg *.png --out converted/"),
    ...
    ("trim a video from 00:01:30 to 00:04:00 without re-encoding", "ffmpeg -ss 00:01:30 -to 00:04:00 -i input.mp4 -c copy trimmed.mp4"),
    ("extract audio track from a video as mp3", "ffmpeg -i video.mp4 -vn -ar 44100 -ac 2 -b:a 192k audio.mp3"),
    ("convert a video to a gif with palette optimization", "ffmpeg -i input.mp4 -vf 'fps=15,scale=640:-1:flags=lanczos,palettegen' palette.png && ffmpeg -i input.mp4 -i palette.png -filter_complex 'fps=15,scale=640:-1:flags=lanczos[x];[x][1:v]paletteuse' output.gif"),
    ("concatenate multiple mp4 files seamlessly", "printf 'file %s\\n' clip1.mp4 clip2.mp4 clip3.mp4 > filelist.txt && ffmpeg -f concat -safe 0 -i filelist.txt -c copy merged.mp4"),
    ("extract a single frame from video at timestamp", "ffmpeg -ss 00:02:15 -i video.mp4 -frames:v 1 -q:v 2 frame.jpg"),
    ("reduce video bitrate for web streaming", "ffmpeg -i input.mp4 -vcodec libx264 -crf 28 -preset slow -acodec aac -b:a 128k web.mp4"),
    ("add subtitles from srt file to video", "ffmpeg -i video.mp4 -vf subtitles=subs.srt output_with_subs.mp4"),
    ("convert and compress a TIFF to JPEG with 85 quality", "convert input.tiff -quality 85 output.jpg"),
    ("batch watermark all images in a directory", "mogrify -draw \"text 10,30 'Copyright 2026'\" -font Helvetica -pointsize 20 -fill 'rgba(255,255,255,0.5)' *.jpg"),
    ("create a contact sheet thumbnail grid from images", "montage *.jpg -geometry 200x150+4+4 -tile 4x contactsheet.jpg"),
    ...
    ("split audio file on silence for track detection", "sox input.wav output.wav silence 1 0.1 0.1% 1 3.0 0.1% : newfile : restart"),
    ("download a YouTube video as best quality mp4", "yt-dlp -f 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]' -o '%(title)s.%(ext)s' 'https://youtu.be/VIDEO_ID'"),
    ("extract only audio from a YouTube playlist as mp3", "yt-dlp -x --audio-format mp3 --audio-quality 0 -o '%(playlist_index)02d - %(title)s.%(ext)s' 'https://www.youtube.com/playlist?list=PLAYLIST_ID'"),
    ("download subtitles for a video without downloading video", "yt-dlp --skip-download --write-subs --sub-lang en 'https://youtu.be/VIDEO_ID'"),
    ("merge multiple PDFs into one with ghostscript", "gs -dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -sOutputFile=merged.pdf part1.pdf part2.pdf part3.pdf"),
    ("extract pages 3 through 7 from a PDF using qpdf", "qpdf --empty --pages input.pdf 3-7 -- extracted.pdf"),
    ("split a PDF into one file per page", "qpdf --split-pages input.pdf page_%d.pdf"),
    ("extract all IPv4 addresses from a file using PCRE", "grep -oP '(?<![0-9])(?:[0-9]{1,3}\\.){3}[0-9]{1,3}(?![0-9])' file.txt | sort -u"),
    ("match lines containing a word boundary with PCRE", "grep -P '\\bfoo\\b' file.txt"),
    ("find lines where a number is followed by specific text using lookahead", "grep -P '[0-9]+(?= errors found)' build.log"),
    ("extract content inside double quotes using named capture", "grep -oP '\"(?P<value>[^\"]+)\"' config.json | grep -oP '(?<=\")[^\"]+(?=\")'"),
    ...
    ("extract a capture group using sed extended regex", "sed -nE 's/.*version[[:space:]]+([0-9.]+).*/\\1/p' Makefile"),
    ...
    ("sum field 2 only for lines matching a pattern", "awk '/ERROR/ {sum += $2} END {print sum}' app.log"),
    ("reformat columns rearranging field order", "awk -F',' '{print $3, $1, $2}' OFS='|' data.csv"),
    ("in-place replace with perl across multiple files", "perl -pi -e 's/old_api_endpoint/new_api_endpoint/g' **/*.ts"),
    ("extract and print a named capture group with perl", "perl -nE 'say $+{host} if /host=(?<host>[\\w.]+)/' config.txt"),
    ("delete lines matching a pattern in-place with perl", "perl -ni -e 'print unless /^#/' config.py"),
    ("use perl lookahead to match word not followed by suffix", "perl -nE 'say if /foo(?!bar)/' file.txt"),
    ("transliterate characters in-place with perl", "perl -pi -e 'tr/a-z/A-Z/' file.txt"),
    ("use perl to join continuation lines ending with backslash", "perl -0777 -pe 's/\\\\\\n//g' Makefile"),
    ("prepend a directory to PATH for the current session only", "export PATH=\"/usr/local/opt/ruby/bin:$PATH\""),
    ("add a directory to PATH permanently in zsh", "echo 'export PATH=\"$HOME/.local/bin:$PATH\"' >> ~/.zshrc && source ~/.zshrc"),
    ("remove a specific entry from PATH without restarting shell", "export PATH=$(echo \"$PATH\" | tr ':' '\\n' | grep -v '/usr/local/opt/old' | tr '\\n' ':' | sed 's/:$//')"),
    ("show all directories in PATH one per line", "echo \"$PATH\" | tr ':' '\\n'"),
    ("check which binary in PATH would run for a command", "type -a python3"),
    ("define a persistent alias in zsh", "echo \"alias ll='ls -lAh --color=auto'\" >> ~/.zshrc && source ~/.zshrc"),
    ("define a shell function to make and enter a directory", "mkcd() { mkdir -p \"$1\" && cd \"$1\"; }"),
    ("define a function to activate a python venv if it exists", "activate() { [[ -f .venv/bin/activate ]] && source .venv/bin/activate || echo 'no .venv found'; }"),
    ("unset an alias that is shadowing a system command", "unalias ls"),
    ("list all currently defined shell functions", "declare -f"),
    ("run cleanup function on script exit regardless of success", "trap 'rm -f /tmp/lockfile.$$; echo cleaned up' EXIT"),
    ("catch SIGINT in a script and exit gracefully", "trap 'echo; echo \"Interrupted — exiting\"; exit 130' INT"),
    ("re-enable default SIGPIPE handling after suppressing it", "trap '' PIPE"),
    ("print error line number on any command failure", "trap 'echo \"Error at line $LINENO\" >&2' ERR"),
    ...
    ("search zsh history interactively with fzf", "history 0 | fzf --tac --no-sort | awk '{$1=\"\"; print $0}' | xargs"),
    ("append current session history to history file immediately", "history -a"),
    ("increase zsh history size and avoid duplicates", "echo 'HISTSIZE=100000; SAVEHIST=100000; setopt HIST_IGNORE_ALL_DUPS SHARE_HISTORY' >> ~/.zshrc"),
    ...
    ("export a variable so all child processes inherit it", "export DATABASE_URL='postgresql://user:pass@localhost:5432/mydb'"),
    ...
    ("watch a GitHub Actions run in real time", "gh run watch $(gh run list --limit 1 --json databaseId -q '.[0].databaseId')"),
    ...
    ("group JSON array by a field and count", "jq 'group_by(.status) | map({status: .[0].status, count: length})' records.json"),
    ("transform JSON with conditional field renaming", "jq '[.[] | {id, name: (.fullName // .name), active: (.status == \"enabled\")}]' users.json"),
    ("merge JSON objects from two files at the key level", "jq -n --slurpfile a a.json --slurpfile b b.json '$a[0] * $b[0]'"),
    ("extract nested paths that match a pattern", "jq '.. | strings | select(test(\"error\"; \"i\"))' response.json"),
    ...
    ("capture packets on port 443 and write to pcap file", "tcpdump -i eth0 -n 'tcp port 443' -w capture.pcap"),
    ("capture DNS queries and show hostnames", "tcpdump -i any -n 'udp port 53' -l 2>/dev/null | grep -oP 'A\\? \\K[^ ]+'"),
    ("watch HTTP POST requests in real time", "tcpdump -i eth0 -A -s 0 'tcp port 80 and (tcp[((tcp[12:1] & 0xf0) >> 2):4] = 0x504f5354)' 2>/dev/null"),
    ("capture packets between two specific hosts", "tcpdump -i eth0 -n 'host 10.0.0.1 and host 10.0.0.2' -w host-pair.pcap"),
    ("allow established and related connections in iptables", "iptables -A INPUT -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT"),
    ("rate-limit incoming SSH connections", "iptables -A INPUT -p tcp --dport 22 -m conntrack --ctstate NEW -m limit --limit 3/min --limit-burst 3 -j ACCEPT"),
    ("list nftables ruleset", "nft list ruleset"),
    ("block outgoing traffic to a specific IP with nftables", "nft add rule ip filter output ip daddr 203.0.113.0/24 drop"),
    ...
    ("measure full HTTP request timing breakdown", "curl -so /dev/null -w 'namelookup:%{time_namelookup}s connect:%{time_connect}s starttransfer:%{time_starttransfer}s total:%{time_total}s\\n' https://example.com"),
    ("check which TLS version and cipher a server uses", "curl -svo /dev/null https://example.com 2>&1 | grep -E 'SSL|TLS|cipher'"),
    ("send request with custom Host header for vhost testing", "curl -sf -H 'Host: staging.example.com' http://10.0.0.5/healthz"),
    ("follow redirects and print each redirect URL", "curl -sIL https://short.link/abc 2>&1 | grep -i 'location:'"),
    ...
    ("run mtr in report mode and show 100 cycles", "mtr --report --report-cycles 100 --no-dns 8.8.8.8"),
    ("traceroute using TCP SYN instead of UDP for firewalled paths", "traceroute -T -p 443 example.com"),
    ...
    ("show me which containers crashed recently", "docker ps -a --filter 'status=exited' --format 'table {{.Names}}\\t{{.Status}}'"),
    ...
    ("what's my CPU temperature right now", "sudo powermetrics --samplers smc -i1 -n1 2>/dev/null | grep 'CPU die'"),
    ("check how long my Mac has been on", "uptime"),
    ("show GPU memory usage", "sudo powermetrics --samplers gpu_power -i1 -n1 2>/dev/null | grep -i gpu"),
    ...
    ("show memory usage by app", "ps aux --sort=-%mem | awk 'NR<=11{print $4\"% \"$11}' | column -t"),
    ("monitor live CPU per core", "sudo powermetrics --samplers cpu_power -i1000 -n3 2>/dev/null | grep -E 'CPU|%'"),
    ("what's my Mac's model and specs", "system_profiler SPHardwareDataType | grep -E 'Model|Memory|Processor'"),
    ...
    ("upgrade all pip packages", "pip list --outdated --format=json | python3 -c \"import json,sys; [print(p['name']) for p in json.load(sys.stdin)]\" | xargs pip install -U"),
    ("show me what brew services are running", "brew services list"),
    ("restart a brew service", "brew services restart postgresql@16"),
    ("check if a formula is installed", "brew list ripgrep 2>/dev/null && echo installed || echo not installed"),
    ...
    ("pull down a remote branch", "git fetch origin && git checkout -b feature/name origin/feature/name"),
    ...
    ("ask amp to review the pull request", "amp 'review the open pull request for security issues'"),
    ...
    ("ask codex to refactor this module", "codex 'refactor src/database.ts to use connection pooling'"),
    ("start codex with full auto approval", "codex --approval-mode full-auto 'add error handling to all API routes'"),
    ("ask codex to explain this codebase", "codex 'give me an overview of the project architecture'"),
    ...
    ("get a council decision on the architecture", "council 'should we use Redis or an in-memory cache here'"),
    ("deploy to Cloud Run from source", "gcloud run deploy cloudagi --source . --region us-central1 --allow-unauthenticated"),
    ("check Cloud Run logs for my service", "gcloud run services logs read cloudagi --region us-central1 --limit 50"),
    ...
    ("authenticate gcloud with application default credentials", "gcloud auth application-default login"),
    ("describe a Cloud Run service", "gcloud run services describe cloudagi --region us-central1"),
    ("update Cloud Run traffic to the latest revision", "gcloud run services update-traffic cloudagi --to-latest --region us-central1"),
    ("list Cloud Run revisions", "gcloud run revisions list --service cloudagi --region us-central1"),
    ("build and push a container image to Artifact Registry", "gcloud builds submit --tag gcr.io/my-project/myapp:latest ."),
    ...
    ("start a tmux session on the remote server", "ssh -t ground-control.online 'tmux attach -t main 2>/dev/null || tmux new -s main'"),
    ("open a remote shell in my main tmux session", "ssh -t ground-control.online 'tmux new-window'"),
    ("run a command on the remote server without staying connected", "ssh ground-control.online 'df -h && free -m'"),
    ("check what is running on the server", "ssh ground-control.online 'ps aux | head -20'"),
    ("tail the app logs on the remote server", "ssh ground-control.online 'tail -f /var/log/app/app.log'"),
    ...
    ("sync my project to the server excluding node_modules", "rsync -avz --exclude node_modules --exclude .git ./ arya@ground-control.online:~/project/"),
    ("sync server logs to local for analysis", "rsync -avz arya@ground-control.online:/var/log/app/ ./logs/"),
    ("dry run rsync to preview what would be transferred", "rsync -avzn --exclude node_modules ./ arya@ground-control.online:~/project/"),
    ("expose local port to internet with cloudflared", "cloudflared tunnel --url http://localhost:3000"),
    ("expose local development server on port 8080", "cloudflared tunnel --url http://localhost:8080"),
    ...
    ("run a model and pass a prompt directly", "ollama run llama3.2 'explain what this error means'"),
    ...
    ("show only the name and image of each docker container", "docker ps --format '{{.Names}}	{{.Image}}'"),
    ("show only the name and status of docker containers", "docker ps --format '{{.Names}}	{{.Status}}'"),
    ("get the IP address of a docker container", "docker inspect mycontainer | jq -r '.[0].NetworkSettings.IPAddress'"),
    ("get the environment variables of a docker container", "docker inspect mycontainer | jq -r '.[0].Config.Env[]'"),
    ("show pod names and their status from kubectl", "kubectl get pods -o json | jq -r '.items[] | [.metadata.name, .status.phase] | @tsv'"),
    ("get just the cluster server URL from kubeconfig", "kubectl config view --minify -o jsonpath='{.clusters[0].cluster.server}'"),
    ("show only failed GitHub Actions runs", "gh run list --limit 20 --json status,name | jq -r '.[] | select(.status==\"failure\") | .name'"),
    ("get the latest npm version of a package", "npm view react version"),
    ("pull the access_token from a curl oauth response", "curl -s https://api.example.com/token -d 'grant_type=client_credentials' | jq -r '.access_token'"),
    ...
    ("extract image names from a docker-compose config", "docker-compose config | yq '.services[].image' | sort -u"),
    ...
    ("show only open files and their PIDs from lsof on port 3000", "lsof -i :3000 | awk 'NR>1 {print $2, $1}'"),
    ("show only the second column from a CSV file", "cut -d, -f2 data.csv"),
    ("extract the third field from a colon-delimited file", "cut -d: -f3 /etc/passwd | sort -n | head -20"),
    ...
    ("show fatal log lines with 3 lines of surrounding context", "grep -C3 'FATAL' /var/log/syslog | tail -40"),
    ...
    ("extract just the version number from a package.json", "sed -n 's/.*\"version\": \"\\(.*\\)\".*/\\1/p' package.json"),
    ("pull out all URLs from an HTML file", "grep -oE 'https?://[^\"]+' index.html | sort -u"),
    ("show only non-comment non-blank lines from a config file", "grep -v '^[[:space:]]*#' /etc/ssh/sshd_config | grep -v '^$'"),
    ("show the 10 largest directories in the current folder", "du -sh */ 2>/dev/null | sort -rh | head -10"),
    ("show the 5 biggest items here", "du -sh * 2>/dev/null | sort -rh | head -5"),
    ("show the largest files anywhere under this directory", "find . -type f -print0 | xargs -0 du -sh | sort -rh | head -15"),
    ...
    ("count lines in all TypeScript files and rank them", "find . -name '*.ts' | xargs wc -l | sort -rn | head -10"),
    ...
    ("show the most common log levels in a log file", "awk '{print $3}' app.log | sort | uniq -c | sort -rn"),
    ("find the most frequently occurring IP addresses in a log", "awk '{print $1}' access.log | sort | uniq -c | sort -rn | head -10"),
    ("show which error messages appear most often", "grep 'ERROR' app.log | awk -F'ERROR' '{print $2}' | sort | uniq -c | sort -rn | head -10"),
    ("show the top 10 git files touched most often", "git log --name-only --pretty='' | grep -v '^$' | sort | uniq -c | sort -rn | head -10"),
    ...
    ("show git log as one line per commit with dates", "git log --format='%h %ad %s' --date=short -20"),
    ...
    ("show only the files changed in the last 5 commits", "git log --name-only --pretty='' -5 | grep -v '^$' | sort -u"),
    ("show commits grouped by author", "git shortlog -sn --no-merges | head -10"),
    ("show docker images with just name and size", "docker images --format '{{.Repository}}:{{.Tag}}	{{.Size}}'"),
    ("list kubernetes pods with just name, status, and node", "kubectl get pods -o custom-columns='NAME:.metadata.name,STATUS:.status.phase,NODE:.spec.nodeName'"),
    ("show file size in bytes only", "stat -f '%z' myfile.txt"),
    ("show file modification time only", "stat -f '%Sm' -t '%Y-%m-%d %H:%M' myfile.txt"),
    ("show df output with aligned columns and human sizes", "df -h | awk 'NR==1 || /^\\//' | column -t"),
    ("show network interface names and their IPs only", "ifconfig | awk '/^[a-z]/{iface=$1} /inet /{print iface, $2}' | column -t"),
    ("print a summary table of file extensions and counts", "find . -type f | sed 's/.*\\.//' | sort | uniq -c | sort -rn | awk '{printf \"%-6s %s\\n\", $1, $2}' | head -15"),
    ("show load average compact", "uptime | awk -F'load averages:' '{print \"Load:\", $2}'"),
    ("show memory usage as a one-liner on macOS", "vm_stat | awk '/free/ {free=$3} /active/ {act=$3} END {printf \"Free: %.1fGB  Active: %.1fGB\\n\", free*4096/1e9, act*4096/1e9}'"),
    ("show running container names on one comma-separated line", "docker ps --format '{{.Names}}' | paste -sd, -"),
    ...
    ("count total lines of TypeScript across the project", "find . -name '*.ts' -not -path '*/node_modules/*' | xargs wc -l | tail -1"),
    ...
    ("count all JSON files in the project excluding node_modules", "find . -name '*.json' -not -path '*/node_modules/*' | wc -l"),
    ("show commit counts by author", "git shortlog -sn --no-merges"),
    ("show how many commits each person made this month", "git shortlog -sn --no-merges --since='1 month ago'"),
    ("show the most common HTTP status codes in an access log", "awk '{print $9}' access.log | sort | uniq -c | sort -rn"),
    ("count how many times each test file was changed in git", "git log --name-only --pretty='' | grep '\\.test\\.' | sort | uniq -c | sort -rn | head -10"),
    ("count open GitHub issues by label", "gh issue list --limit 200 --json labels | jq '[.[].labels[].name] | group_by(.) | map({label: .[0], count: length}) | sort_by(-.count)[]'"),
    ("show how many lines each contributor added in git", "git log --numstat --pretty='' | awk '{add+=$1} END {print \"Lines added:\", add}'"),
    ...
    ("check if a brew package is installed without extra output", "brew list --formula | grep -q '^ripgrep$' && echo installed || echo missing"),
    ("run make and suppress all error output", "make build 2>/dev/null"),
    ("suppress permission errors when searching files", "find / -name '*.conf' 2>/dev/null | head -20"),
    ("check if a command exists without any output", "command -v docker &>/dev/null && echo yes || echo no"),
    ...
    ("show ps output without the header sorted by memory", "ps aux | tail -n +2 | sort -k4 -rn | head -10"),
    ("strip the header row from a CSV before processing", "tail -n +2 data.csv | cut -d, -f1,3 | sort"),
    ("quick system summary on macOS", "echo \"CPU: $(sysctl -n hw.ncpu) cores  RAM: $(sysctl -n hw.memsize | awk '{printf \"%.0fGB\", $1/1073741824}')  Disk: $(df -h / | awk 'NR==2{print $5}') used  Load: $(sysctl -n vm.loadavg | awk '{print $2}')\""),
    ("show a compact project health summary", "echo \"Tests: $(find . -name '*.test.ts' | wc -l | tr -d ' ')  TS files: $(find . -name '*.ts' -not -path '*/node_modules/*' | wc -l | tr -d ' ')  LOC: $(find . -name '*.ts' -not -path '*/node_modules/*' | xargs wc -l 2>/dev/null | tail -1 | awk '{print $1}')\""),
    ("show a compact git repo status", "echo \"Branch: $(git branch --show-current)  Commits: $(git rev-list --count HEAD)  Dirty: $(git status --porcelain | wc -l | tr -d ' ') files\""),
    ("quick docker status summary", "echo \"Running: $(docker ps -q | wc -l | tr -d ' ')  Stopped: $(docker ps -qa --filter status=exited | wc -l | tr -d ' ')  Images: $(docker images -q | wc -l | tr -d ' ')\""),
    ("show my external and internal IP on one line", "echo \"External: $(curl -s ifconfig.me)  Internal: $(ipconfig getifaddr en0)\""),
    ("compact node project info", "node -e \"const p=require('./package.json'); console.log(p.name+'@'+p.version, '| node:'+process.version)\""),
    ("show a one-line process and memory snapshot", "ps aux | awk 'NR>1 {mem+=$4; cpu+=$3; count++} END {printf \"Processes: %d  Avg CPU: %.1f%%  Avg Mem: %.1f%%\\n\", count, cpu/count, mem/count}'"),
    ("show top 3 CPU and memory consumers on one screen", "echo '--- CPU ---' && ps aux --sort=-%cpu | awk 'NR>1 && NR<5 {print $3\"% \"$11}' && echo '--- MEM ---' && ps aux --sort=-%mem | awk 'NR>1 && NR<5 {print $4\"% \"$11}'"),
    ("compact kubernetes cluster summary", "echo \"Nodes: $(kubectl get nodes --no-headers | wc -l | tr -d ' ')  Pods: $(kubectl get pods --all-namespaces --no-headers | wc -l | tr -d ' ')  Services: $(kubectl get svc --all-namespaces --no-headers | wc -l | tr -d ' ')\""),
    ("show current directory summary in one line", "echo \"Files: $(ls | wc -l | tr -d ' ')  Dirs: $(ls -d */ 2>/dev/null | wc -l | tr -d ' ')  Size: $(du -sh . 2>/dev/null | cut -f1)  Git: $(git branch --show-current 2>/dev/null || echo none)\""),
    ...
    ("see how much disk space homebrew cleanup would free without deleting anything", "brew cleanup -n"),
    ...
    ("clean node_modules and reinstall everything from scratch", "rm -rf node_modules && bun install"),
    ...
    ("scaffold a new vite project without globally installing vite", "npx create-vite@latest my-app"),
    ...
    ("what kind of project is this", "ls package.json Cargo.toml pyproject.toml Makefile go.mod 2>/dev/null"),
    ("show project dependencies based on whatever system this uses", "{ [ -f package.json ] && jq '.dependencies' package.json; } || { [ -f Cargo.toml ] && cargo tree; } || { [ -f pyproject.toml ] && uv tree; }"),
    ("lint the project using whichever linter is configured", "{ [ -f package.json ] && bun run lint; } || { [ -f Cargo.toml ] && cargo clippy --all-targets; } || { [ -f pyproject.toml ] && uvx ruff check .; }"),
    ("check if the project builds successfully", "{ [ -f package.json ] && bun run build; } || { [ -f Cargo.toml ] && cargo build; } || { [ -f pyproject.toml ] && uv sync; }"),
    ("run the project's test suite using the appropriate tool", "{ [ -f package.json ] && bun test; } || { [ -f Cargo.toml ] && cargo test; } || { [ -f pyproject.toml ] && uv run pytest; }"),
    ...
    ("show what changed since the last release tag", "git log $(git describe --tags --abbrev=0)..HEAD --oneline"),
    ("open the project in the finder", "open ."),
    ("count total lines of source code in this project", "find . -type f \\( -name '*.ts' -o -name '*.rs' -o -name '*.py' \\) -not -path '*/node_modules/*' -not -path '*/.venv/*' | xargs wc -l | tail -1"),
    ("show the size of node_modules", "du -sh node_modules 2>/dev/null || echo 'no node_modules'"),
    ("check the node version required by this project", "jq '.engines.node' package.json 2>/dev/null || cat .nvmrc 2>/dev/null || cat .node-version 2>/dev/null"),
    ...
    ("generate a lockfile for the current bun project without installing", "bun install --frozen-lockfile --dry-run"),
    ("show what changed in dependencies since the last lockfile commit", "git diff HEAD -- bun.lockb package.json Cargo.lock uv.lock"),
    ("check if the lockfile is up to date without modifying anything", "bun install --frozen-lockfile"),
    ...
    ("find which dependency introduced a vulnerable package", "npm ls --all 2>/dev/null | grep -i <pkg>"),
    ...
    ("delete all node_modules folders", "find . -name node_modules -type d -prune -exec rm -rf {} +"),
    ...
    ("find all the TODO comments in the code", "rg --line-number 'TODO|FIXME|HACK' --glob '*.{ts,js,py}'"),
    ...
    ("which branch has this fix", "git branch --contains $(git log --all --oneline | grep 'fix description' | awk '{print $1}')"),
    ...
    ("find the commit that broke things", "git bisect start && git bisect bad HEAD && git bisect good v0.9.0"),
    ...
    ("check my internet speed", "curl -s https://raw.githubusercontent.com/sivel/speedtest-cli/master/speedtest.py | python3"),
    ("who's logged in", "w"),
    ...
    ("what hardware am I running on", "system_profiler SPHardwareDataType | grep -E 'Model|Chip|Memory'"),
    ("how hot is my CPU", "sudo powermetrics --samplers smc -n1 2>/dev/null | grep 'CPU die temperature'"),
    ...
    ("see how big my images are", "docker images --format 'table {{.Repository}}\\t{{.Tag}}\\t{{.Size}}' | sort -k3 -rh"),
    ...
    ("replace text across all files matching a pattern", "rg pattern -l | xargs sed -i '' 's/old/new/g'"),
    ...
    ("find files modified between two dates", "fd --changed-after 2025-01-01 --changed-before 2025-06-01"),
    ("find all json files and pretty-print them", "fd -e json -x sh -c 'echo \"=== {} ===\"; cat {}'"),
    ...
    ("fuzzy search command history and run selection", "history | fzf | awk '{$1=\"\"; print}' | bash"),
    ("preview files with syntax highlighting while searching", "fd -t f | fzf --preview 'bat --color=always {}'"),
    ("fuzzy search docker containers and tail logs", "docker ps --format '{{.Names}}' | fzf | xargs docker logs -f"),
    ...
    ("fuzzy select an npm script and run it", "jq -r '.scripts | keys[]' package.json | fzf | xargs bun run"),
    ("fuzzy pick a running docker container and exec into it", "docker ps --format '{{.Names}}' | fzf | xargs -I{} docker exec -it {} bash"),
    ("fuzzy select a file and copy its path to clipboard", "fd -t f | fzf | pbcopy"),
    ("fuzzy search environment variables", "env | fzf"),
    ("fuzzy select a git stash and apply it", "git stash list | fzf | awk -F: '{print $1}' | xargs git stash apply"),
    ("fuzzy find and delete a git branch", "git branch | grep -v '\\*' | fzf | xargs git branch -d"),
    ...
    ("count lines of code across TypeScript Python and Rust files", "fd -e ts -e py -e rs -x wc -l | sort -rn"),
    ("show unique error types from a log file", "rg ERROR app.log | awk -F: '{print $NF}' | sort -u"),
    ("find files with duplicate names across directories", "fd -t f | awk -F/ '{print $NF}' | sort | uniq -d"),
    ("show top five most common HTTP status codes in access log", "awk '{print $9}' access.log | sort | uniq -c | sort -rn | head -5"),
    ("fetch JSON from an API and format as an aligned table", "curl -s https://api.example.com/data | jq -r '.[] | [.name, .value] | @tsv' | column -t"),
    ("watch source directory for changes and re-run tests", "fswatch -o src/ | xargs -n1 -I{} bun test"),
    ...
    ("tail logs and highlight ERROR lines in red", "tail -f app.log | rg --passthru --color=always ERROR"),
    ("find the most recently modified file in a directory", "fd -t f | xargs ls -t | head -1"),
    ("batch rename files replacing spaces with underscores", "fd -t f -e txt | xargs -I{} bash -c 'mv \"$0\" \"${0// /_}\"' {}"),
    ("count the number of each file extension in a project", "fd -t f | awk -F. '{print $NF}' | sort | uniq -c | sort -rn"),
    ("show git diff stats piped through bat for syntax highlighting", "git diff --stat | bat -l diff"),
    ...
    ("show the ten largest directories in the current path", "du -sh */ 2>/dev/null | sort -rh | head -10"),
    ...
    ("source env file if it exists otherwise warn", "test -f .env && source .env || echo 'no .env found'"),
    ("feed two sorted files into a join operation", "join <(sort file1) <(sort file2)"),
    ("run a command for each line of a variable", "while IFS= read -r line; do echo \"$line\"; done <<< \"$multiline_var\""),
    ("check if a command exists before running it", "command -v rg &>/dev/null && rg TODO . || grep -r TODO ."),
