...
PAIRS: list[tuple[str, str]] = [
    ...
    ("re-run all failed jobs in the latest workflow run", "gh run rerun --failed $(gh run list --limit 1 --json databaseId -q '.[0].databaseId')"),
    ("view logs for a specific failed run", "gh run view --log-failed $(gh run list --limit 1 --json databaseId -q '.[0].databaseId')"),
    ("list all workflow files in a repo", "gh workflow list"),
    ("manually trigger a workflow with an input", "gh workflow run deploy.yml -f environment=staging"),
    ...
    ("import an existing resource into terraform state", "terraform import aws_s3_bucket.assets my-bucket-name"),
    ...
    ("run ad-hoc shell command on a host group", "ansible webservers -m shell -a 'systemctl status nginx' -i inventory.ini"),
    ("copy a file to all remote hosts", "ansible all -m copy -a 'src=./app.conf dest=/etc/app/app.conf' -i inventory.ini --become"),

    # Make/just patterns
    ("list all available make targets", "make -qp | awk -F':' '/^[a-zA-Z0-9][^$#\\/\\t=]*:([^=]|$)/ {split($1,A,/ /); for(i in A) print A[i]}' | sort -u"),
    ...
    ("run a single test function by node ID", "pytest tests/test_api.py::test_login_returns_token -v"),
    ...
    # Coverage commands
    ("run tests with coverage and show missing lines", "pytest --cov=src --cov-report=term-missing"),
    ("fail the build if coverage drops below 80%", "pytest --cov=src --cov-fail-under=80"),
    ("generate an HTML coverage report", "pytest --cov=src --cov-report=html && open htmlcov/index.html"),
    ("show coverage diff against main branch", "coverage run -m pytest && coverage xml && diff-cover coverage.xml --compare-branch=main"),

    # Lint runner patterns
    ("run ruff with auto-fix on entire project", "ruff check . --fix"),
    ("run eslint on staged files only", "git diff --cached --name-only --diff-filter=ACM | grep '\\.ts$' | xargs eslint"),
    ("run clippy on all targets treating warnings as errors", "cargo clippy --all-targets -- -D warnings"),
    ("lint all Python files and show first error only", "ruff check . --select E,W --quiet | head -1"),

    # Type checker invocations
    ("check types across the whole monorepo with pyright", "pyright --outputjson 2>/dev/null | jq '.summary'"),
    ("run mypy in strict mode on a module", "mypy --strict src/auth.py"),

    # Benchmark runners
    ("run criterion benchmarks and open HTML report", "cargo bench && open target/criterion/report/index.html"),
    ("compare benchmark results against baseline", "hyperfine --warmup 5 --export-markdown bench.md './cmd_old' './cmd_new'"),
    ...
    ("filter CSV rows where column value exceeds threshold", "csvgrep -c revenue -m '' data.csv | csvsort -c revenue -r | head -20"),
    ...
    ("merge two YAML files, second overrides first", "yq eval-all 'select(fileIndex == 0) * select(fileIndex == 1)' base.yaml override.yaml"),
    ...
    ("convert JSON to YAML using python", "python3 -c \"import sys, json, yaml; yaml.dump(json.load(sys.stdin), sys.stdout, default_flow_style=False)\" < data.json"),
    ("base64 encode a file and copy to clipboard", "base64 < secret.key | tr -d '\\n' | pbcopy"),
    ("base64 decode a string and write to file", "echo 'SGVsbG8gV29ybGQ=' | base64 --decode > output.txt"),
